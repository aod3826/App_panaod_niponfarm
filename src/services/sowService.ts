...
export const addSow = async (sowData: Omit<Sow, 'id' | 'status' | 'parity' | 'createdAt' | 'updatedAt' | 'userId'>) => {
  const now = Date.now();
...
export const recordEvent = async (sow: Sow, eventType: EventType, date: string, details: any, relatedTaskId?: string, recordedBy?: string) => {
  const batch = writeBatch(db);
  const now = Date.now();
  const userId = getCurrentUserId();
  ...
  try {
    ...
    // Cancel other pending tasks if aborted, heat return, or cull
    if (eventType === 'HEAT_RETURN' || eventType === 'CULL' || (eventType === 'ULTRASOUND' && details.result !== 'POSITIVE')) {

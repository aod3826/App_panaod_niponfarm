import { collection, query, where, orderBy, onSnapshot, getDocs, setDoc, doc, addDoc, updateDoc, increment, getDoc, writeBatch, limit } from 'firebase/firestore';
...
export const subscribeToMessages = (roomId: string, limitCount: number, callback: (messages: ChatMessage[]) => void) => {
  const q = query(
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    // Sort ascending client-side for display

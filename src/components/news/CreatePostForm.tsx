...
export default function CreatePostForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (videoInputRef.current) videoInputRef.current.value = '';
      if (audioInputRef.current) audioInputRef.current.value = '';
      // Optional: showAlert('โพสต์สำเร็จ');
    } catch (error: any) {
      if (error && error.code === 'storage/unknown') {
        showAlert('ระบบพื้นที่เก็บไฟล์ยังไม่เปิดใช้งาน กรุณาตั้งค่า Firebase Storage ก่อน หรือติดต่อผู้ดูแลระบบ');
      } else {
        showAlert('เกิดข้อผิดพลาดในการโพสต์ กรุณาลองใหม่อีกครั้ง');
      }
    } finally {
      setLoading(false);
    }

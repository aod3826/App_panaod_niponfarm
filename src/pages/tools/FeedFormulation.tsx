...
export default function FeedFormulation() {
  ...
  const handleExportPDF = () => {
    showAlert('กำลังสร้างไฟล์ PDF...', 'ระบบกำลังจัดเตรียมเอกสาร');
    setTimeout(async () => {
      const element = document.getElementById('feed-print-container');
      if (!element) {
        console.error('Element not found: feed-print-container');
        showAlert('เกิดข้อผิดพลาด ไม่พบพื้นที่สำหรับพิมพ์', 'ข้อผิดพลาด');
        return;
      }
      ...
      try {
        ...
        pdf.save(`FeedRecipe_${recipeName ? recipeName.replace(/\s+/g, '_') : 'Untitled'}.pdf`);
        showAlert('ดาวน์โหลด PDF สำเร็จ!');
      } catch (e) {
        console.error('Error generating PDF', e);
        showAlert('เกิดข้อผิดพลาดในการสร้าง PDF', 'ข้อผิดพลาด');
      }
    }, 500);
  };
  ...
  return (
    <>
      ...
      <div style={{ position: 'absolute', top: '-9999px', left: '-9999px', pointerEvents: 'none' }}>
        <div id="feed-print-container" className="bg-white text-black p-10 overflow-auto w-[800px]">
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 dark:bg-[#0a2e36]/80 backdrop-blur-sm">
            <motion.div 

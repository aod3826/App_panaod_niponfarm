...
export default function NewMaintenanceRequest() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showAlert('บันทึกการแจ้งซ่อมเรียบร้อยแล้ว');
      navigate('/maintenance');
    } catch (error: any) {
      console.error('Error creating maintenance request:', error);
      // Errors are already handled by service with handleFirestoreError
      // But we can show a specific alert if needed
      showAlert('ไม่สามารถบันทึกข้อมูลได้ กรุณาตรวจสอบการเชื่อมต่ออินเทอร์เน็ต');
    } finally {
      setLoading(false);
    }
  };

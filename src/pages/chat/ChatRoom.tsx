...
export default function ChatRoomPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToMessages(roomId, limitCount, (msgs) => {
      setMessages(msgs);
      setLoading(false);
    ...
    return () => unsubscribe();
  }, [roomId, user, limitCount]);

  // Auto scroll to bottom only when a new message is received (not when loading older)
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] -mt-6 -mx-4 sm:mx-0 sm:mt-0 bg-gray-50 sm:rounded-3xl sm:border border-gray-200 overflow-hidden relative">
      ...
      <div 
        ...
        className="flex-1 overflow-y-auto p-4 space-y-3 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-fixed" 

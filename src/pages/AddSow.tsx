...
export default function AddSow() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.sowId || !formData.breed) {
      showAlert('กรุณากรอกเบอร์หูและสายพันธุ์');
      return;
    }

    setLoading(true);
    try {
      const resultId = await addSow(formData);
      if (resultId) {
      ...
      showAlert(error.message || 'เกิดข้อผิดพลาดในการบันทึกข้อมูล กรุณาตรวจสอบการเชื่อมต่ออินเทอร์เน็ต');
    } finally {
      setLoading(false);
    }
  };

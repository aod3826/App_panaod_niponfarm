  ...
  return (
    <div className="animate-in fade-in duration-300 relative min-h-[80vh]">
        ...
        <div className="relative group">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3.5 bg-slate-100/90 dark:bg-[#1a2f3a]/90 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-2xl shadow-sm text-slate-900 dark:text-white placeholder-slate-500 dark:placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-[#00bcd4] focus:border-transparent transition-all text-lg"
            />
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-slate-100 dark:bg-white/5 backdrop-blur-md p-8 rounded-3xl border border-slate-200 dark:border-white/10 border-dashed text-center shadow-xl dark:shadow-2xl">
          <div className="w-16 h-16 bg-slate-100 dark:bg-white/5 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-200 dark:border-white/10">
            <Search className="w-8 h-8 text-slate-600 dark:text-white/30" />
          </div>
          <p className="text-slate-600 dark:text-white/70 font-medium text-lg">ไม่พบข้อมูลแม่/พ่อพันธุ์</p>
          <p className="text-sm text-slate-600 dark:text-white/40 mt-1">ลองเปลี่ยนคำค้นหาหรือตัวกรอง</p>
        </div>
      ) : (
        <div className="grid gap-4 pb-24">
          {filteredSows.map((sow) => {
            ...
            <div 
              ...
              className="bg-white dark:bg-[#1a2f3a] p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-white/10 active:scale-[0.98] transition-transform cursor-pointer flex flex-col gap-3"
            >
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">{sow.sowId}</h3>
                    <span className={clsx("px-2.5 py-1 rounded-md text-[11px] font-bold border tracking-wider flex items-center", getStatusColor(sow.status))}>
      ...
      <button 
        ...
        className="fixed bottom-24 right-6 w-16 h-16 bg-[#00bcd4] text-slate-900 dark:text-white rounded-full shadow-[0_0_20px_rgba(0,188,212,0.4)] flex items-center justify-center hover:bg-cyan-400 active:scale-90 transition-all z-10 border border-slate-200 dark:border-white/20"
      >

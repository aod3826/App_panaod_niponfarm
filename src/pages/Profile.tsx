...
export default function Profile() {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 1024 * 1024) { // 1MB limit for Base64 in Firestore doc
        showAlert("ไฟล์รูปภาพต้องมีขนาดไม่เกิน 1MB");
        return;
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showAlert("บันทึกข้อมูลส่วนตัวของคุณเรียบร้อยแล้ว");
      setIsEditing(false);
    } catch (error) {
      console.error("Error updating profile:", error);
      showAlert("ไม่สามารถบันทึกข้อมูลได้ โปรดลองอีกครั้ง");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative mb-8 pt-12 pb-8 px-6 bg-white dark:bg-white/5 rounded-[32px] shadow-xl shadow-blue-500/5 border border-slate-100 dark:border-white/10 overflow-hidden text-center">
        ...
        <div className="relative mb-4 flex flex-col items-center">
          <div className="relative">
            <div className="w-28 h-28 rounded-full bg-white dark:bg-slate-800 p-1.5 shadow-xl border-4 border-white dark:border-slate-800 relative z-10">
               {formData.photoURL || userProfile.photoURL ? (
                 ...
                 <div className="w-full h-full rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-4xl font-black text-blue-600 uppercase">
